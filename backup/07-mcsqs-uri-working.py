...
def create_mcq_pdf(assessment: Assessment, filename: str):
    ...
    # Define a function to build each MCQ as a paragraph
    def build_mcq_paragraph(mcq: MCQ, index: int) -> Paragraph:
        text = f"{index + 1}. {mcq.question}<br/>"
        text += f" a. {mcq.options[0]}<br/>"
        text += f" b. {mcq.options[1]}<br/>"
        text += f" c. {mcq.options[2]}<br/>"
        text += f" d. {mcq.options[3] if len(mcq.options) > 3 else 'N/A'}<br/>"
        text += "<br/>"  # Add a blank line between questions
