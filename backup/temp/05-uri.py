...
#from langchain_community.document_loaders import PyPDFLoader  # Removed local file loading
#from langchain_community.document_loaders import GCSFileLoader # Added GCS loader - DEPRECATED
# from langchain_google_cloud.storage import GCSFileLoader
...
def load_pdf_content_from_gcs(gcs_uri: str) -> PDFContent:
    ...
    try:
        ...
        # loader = GCSFileLoader(bucket=bucket_name, blob=blob_name)
...
def extract_pdf_metadata_from_gcs(gcs_uri: str) -> Optional[PDFMetadata]:
    """Extracts metadata from a PDF document stored in Google Cloud Storage."""
    try:
...
if __name__ == "__main__":
    gcs_pdf_uri = "gs://bhagavan-pub-bucket/aignite-resources/jemh1a1.pdf"

    metadata = extract_pdf_metadata_from_gcs(gcs_pdf_uri)
